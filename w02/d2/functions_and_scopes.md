...
When declaring variables using the ES6 `let` and `const` initializers inside of a code block, these variables have *block scope*, meaning the variables are only recognized in the scope of the block they have been created (or declared) in.
